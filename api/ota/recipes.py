...
def _appimage(p: dict) -> str:
    ...
    return "\n".join([
        ...
        "# Ein AppImage haengt sich normalerweise beim Start selbst per FUSE",
        "# ein. Das braucht CAP_SYS_ADMIN, und die bekommt ein Arbeitsplatz",
        "# nicht — sie ist praktisch gleichbedeutend mit root auf dem Host.",
        "# AppImages koennen sich stattdessen entpacken und daraus starten;",
        "# das kostet einen Moment beim Start und sonst nichts.",
        f"cat > /usr/local/bin/{name} <<'RUN'",
        "#!/bin/sh",
        "export APPIMAGE_EXTRACT_AND_RUN=1",
        f'exec /opt/{name}.AppImage "$@"',
        "RUN",
